...
const ProductList = () => {
  ...
  // Check if property is a number-like string
  const isNumeric = (n) => !isNaN(parseFloat(n)) && isFinite(n);

  // Apply sorting
  const sortedProducts = [...filteredProducts].sort((a, b) => {
    if (!a[sortConfig.key] && sortConfig.key !== 'stock' && sortConfig.key !== 'sales' && !b[sortConfig.key]) return 0;
    ...
    if (sortConfig.key === 'stock') {
      valA = a.stock !== undefined ? a.stock : (a.countInStock || 0);
      valB = b.stock !== undefined ? b.stock : (b.countInStock || 0);
  ...
  return (
    <div className="seller-product-list">
      <div className="list-header">
        <h2>Products</h2>
        <button className="btn-add-product" onClick={() => navigate('/list-your-product')}>
          ADD PRODUCT
        </button>
      </div>
          ...
          <table className="modern-table">
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort('sales')} style={{ cursor: 'pointer' }}>
            ...
            <tbody>
              {sortedProducts.map(p => (
                <tr key={p._id}>
                  ...
                  <td>{p.sales || 0} sale</td>
                  <td>
                    <span className="stock-val">
                      {p.stock !== undefined ? p.stock : (p.countInStock !== undefined ? p.countInStock : 0)}
                    </span>
                  </td>
                  <td>
                  ...
                  <td className="action-cell">
                    <button className="action-btn" title="Edit" onClick={() => setEditingProduct(p)}>
                      <Pencil size={18} />
                    </button>
